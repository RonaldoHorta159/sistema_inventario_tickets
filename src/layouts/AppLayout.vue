...
// 1. Datos Reactivos (State)
const isSidebarOpen = ref(true); // Booleano para abrir/cerrar menú

// 2. Estructura de Datos del Menú (Array de Objetos)
// Aquí defines tu navegación. Es fácil de mantener: si quieres otro link, agregas un objeto.
const menuItems = [
...
// 3. Funciones (Methods)
const toggleSidebar = () => {
    isSidebarOpen.value = !isSidebarOpen.value; // Invierte el valor (true -> false)
};

const router = useRouter();
const logout = () => {
    // Aquí luego pondremos la lógica de Supabase
...
<template>
    <div class="min-h-screen flex bg-gray-50">
        ...
        <aside 
            class="bg-white border-r border-gray-200 transition-all duration-300 flex flex-col fixed h-full z-20"
        ...
        >
            ...
            <nav class="flex-1 py-4 px-2 space-y-1">
                ...
                >
                    ...
                    <div v-if="!isSidebarOpen" class="absolute left-16 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity">
                        {{ item.label }}
                    </div>
                </router-link>
            </nav>
            ...
            <div class="p-4 border-t border-gray-100">
                <button @click="logout" class="flex items-center w-full px-4 py-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
        ...
        <div class="flex-1 flex flex-col transition-all duration-300" :class="isSidebarOpen ? 'ml-64' : 'ml-20'">
            ...
            <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 sticky top-0 z-10 shadow-sm">
                <div class="flex items-center">
                    <button @click="toggleSidebar" class="p-2 rounded-full hover:bg-gray-100 text-gray-600 focus:outline-none">
